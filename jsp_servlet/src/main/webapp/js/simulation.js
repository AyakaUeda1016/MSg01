...
function calculateVoiceFeatureScores(emotionRaw) {
  ...
  const loudnessScore = Math.max(0, 1 - Math.abs(rawLoud - idealLoud) / loudTol) * 100
  ...
  const warmthScoreRaw = Math.max(0, 1 - Math.abs(warmth - warmthIdeal) / warmthTol) * 100
...
function calcDelta(prev, curr) {
  ...
  const dArousal = Math.abs((curr.arousal ?? 0) - (prev.arousal ?? 0)) / 0.1

  const dPitch = Math.abs((curr.pitch_variability ?? 0) - (prev.pitch_variability ?? 0)) / 0.08

  const dLoud = Math.abs((curr.loudness_variability ?? 0) - (prev.loudness_variability ?? 0)) / 0.12
...
// ★ POP 優先度判定（Level2）
//======================================================
function decidePopupToShow(result, meterState, prevMeterSnapshot) {
  let popupMessage = null

  // ★ POP を出していいかどうか
  const allowPopup = result.turn !== 4   // turn=4 は happy確定ターン

  // 優先度①：無関係
  if (allowPopup && result.appropriateness === "unrelated") {
    popupMessage = "今はこの話題について話しています。内容を戻してみましょう。"
  }

  // 優先度②：不適切
  else if (allowPopup && result.appropriateness === "related_bad") {
    popupMessage = "優しい言葉を心掛けましょう"
  }

  // 優先度③：緊張度
  else if (
    allowPopup &&
    prevMeterSnapshot.tension < 70 &&
    meterState.tension >= 70
  ) {
    popupMessage = "緊張しているかも"
  }

  // 優先度④：声量
  else if (allowPopup && meterState.voice_loudness <= 30) {
    popupMessage = "もう少しはっきり話してみよう"
  }

  // 優先度⑤：思いやり
  else if (
    allowPopup &&
    typeof result.empathy_score === "number" &&
    result.empathy_score <= 40
  ) {
    popupMessage = "相手への配慮が伝わりにくいかも"
  }
...
function updateEmotionDisplay(emotion) {
  ...
  container.classList.remove("emotion-happy", "emotion-sad", "emotion-angry", "emotion-default")
...
function updateDisplayFromFlask(result) {
  ...
  // ===== ② 内容の状態を決める（減衰用）=====
  const contentState = result.appropriateness === "related_bad" ? "bad" : "normal"

  // ===== (確認用）：delta のログ=====
  console.log("[DELTA CHECK]", {
    delta: delta,
    step: delta >= 1.5 ? 2 : delta >= 0.8 ? 1 : 0,
  ...
  // ★ POP表示（優先度制御・ここだけ）
  const popupMessage = decidePopupToShow(result, meterState, prevMeterSnapshot)
  if (popupMessage) {
  ...
  if ((result.turn >= MAX_TURNS || result.active === false) && !isConversationFinished) {
    console.log("[SESSION] conversation finished, start finish loading...")
...
function updateSkillScoresDisplay(voiceScores) {
  ...
  // ★ 内部スコアも必ず更新（保存・総合用）
  const prevS1 = skillScores.selfUnderstanding
  const prevS2 = skillScores.comprehension

  skillScores.selfUnderstanding = s1
  skillScores.comprehension = s2

  updateMeterWithAnimation(selfUnderstandingMeter, prevS1, s1)
  updateMeterWithAnimation(comprehensionMeter, prevS2, s2)
}

function updateMeterWithAnimation(meterElement, prevValue, newValue) {
  if (!meterElement) return

  const container = meterElement.parentElement
  const scoreElement = container?.querySelector(".skill-score")

  if (container) {
    container.classList.remove("ripple-effect")
  ...
  meterElement.classList.remove("meter-increase", "meter-decrease")

  if (newValue > prevValue) {
    meterElement.classList.add("meter-increase")
  } else if (newValue < prevValue) {
    meterElement.classList.add("meter-decrease")
  }

  if (scoreElement && prevValue !== newValue) {
    animateScore(scoreElement, prevValue, newValue, 800)

    scoreElement.classList.remove("score-changed")
    void scoreElement.offsetWidth // リフロー強制
  ...
  // メーターの幅を更新
  meterElement.style.width = `${newValue}%`
